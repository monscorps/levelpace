...
function Invoke-Safe([scriptblock] $work, [string] $what) {
    try {
        ...
        # Name the statement, not just the message. "Cannot find drive
        # 'https'" once appeared in a real log exactly once, with nothing to
        # say which of forty path operations had been handed a URL. Under
        # Invoke-Expression line numbers mean nothing; the source line and the
        # calling function do.
        try {
            $line = $_.InvocationInfo.Line
            $stack = @(($_.ScriptStackTrace -split "`r?`n") | Where-Object { $_ } | Select-Object -First 2)
            $at = @()
            if ($line) { $at += $line.Trim() }
            if ($stack.Count) { $at += ($stack -join ' < ') }
            if ($at.Count) { $msg = $msg + '  [at: ' + ($at -join ' | ') + ']' }
        } catch { }
...
function Get-Config {
    ...
    $token  = $null
    $base   = $null
    # The READ address is the line that names the board's baseline file. It
    # used to be "the first URL after the send line" -- which, for as long as
    # the retired local server's address lingered in server.txt, was THAT
    # address. Every board fetch dialled a machine that no longer existed and
    # logged "cannot connect" five times an hour while uploads worked fine.
    $urls = @()
    foreach ($v in $vals[1..([Math]::Max(1, $vals.Count - 1))]) {
        if ($v -match '^https?://') { $urls += $v }
        elseif (-not $token) { $token = $v }
    }
    $base = ($urls | Where-Object { $_ -match '/api/baseline\.json$' } | Select-Object -First 1)
    if (-not $base -and $urls.Count) { $base = $urls[0] }
...
function Find-WowRoots {
    ...
    # Callers wrap this in @(). A list with ONE entry unrolls to a bare string
    # on the way out, and then $roots[0] is its first CHARACTER -- which is
    # how a real log came to say "Found WoW at D." and, from that, "the addon
    # is not installed there". (Not fixed here with `return ,@(...)`: combined
    # with the callers' @() that nests the array and breaks them the other way.)
...
# First non-empty property out of several names. Two servers have called the
# same thing 'name' and 'display', 'percentile' and 'parse'; the file written
# for the game carries both so the reader never has to know which wrote it.
function Pick {
    param($obj)
    foreach ($n in $args) {
        $v = $obj.$n
...
function Write-Board($addonDir, $overall, $twinks, $version, $source) {
    ...
    foreach ($e in @($overall) | Select-Object -First 100) {
        $pct = Pick $e 'percentile' 'pct' 'parse'
        $met = Pick $e 'metric' 'best'
        $l.Add('    { rank=' + (LuaNum $e.rank) + ', name=' + (LuaStr (Pick $e 'name' 'display')) +
               ', realm=' + (LuaStr $e.realm) + ', class=' + (LuaStr $e.class) +
               ', level=' + (LuaNum $e.level) + ', metric=' + (LuaNum $met) +
               ', percentile=' + (LuaNum $pct) + ', band=' + (LuaStr $e.band) +
               ', parse=' + (LuaNum $pct) + ', levels=' + (LuaNum $e.levels) +
               ', best=' + (LuaNum $met) + ' },')
    }
    $l.Add('  },')
    $l.Add('  twinks = {')
    foreach ($e in @($twinks) | Select-Object -First 100) {
        ...
        $l.Add('    { rank=' + (LuaNum $e.rank) + ', name=' + (LuaStr (Pick $e 'name' 'display')) +
               ', realm=' + (LuaStr $e.realm) + ', class=' + (LuaStr $e.class) +
               ', level=' + (LuaNum $e.level) + ', metric=' + (LuaNum $e.metric) +
               ', percentile=' + (LuaNum $pct) + ', band=' + (LuaStr $e.band) +
               ', bracket=' + (LuaNum $e.bracket) + ', ilvl=' + (LuaNum $e.item_level) +
               ', weekly=' + (LuaNum $e.weekly_kills) + ', lifetime=' + (LuaNum (Pick $e 'lifetime_kills' 'metric')) +
               ', deaths=' + (LuaNum $e.deaths) + ', kills=' + (LuaNum (Pick $e 'kills' 'metric')) +
...
function Invoke-Sync([switch]$Quiet) {
    ...
    if ($payloads.Count -eq 0) {
        ...
        try {
            ...
            # The heartbeat used to go out ONLY when there was nothing to send,
            # so the client table froze at "addon wrote no blob" for every
            # install that had long since started uploading fine.
            $av = Get-AddonVersion
            Send-Hello $cfg $true $true $true ("uploaded {0} level(s) [addon {1}, companion {2}]" -f `
                $sent, $(if ($av) { $av } else { '?' }), $Version)
        } catch {
            $ok = $false
    ...
    try {
        # Rankings and the baseline come from the same place the upload went:
        # the Worker recomputes them on every write, so what comes back is
        # exactly what the web board shows. Only version.json is static -- it
        # sits beside the web board and the release script stamps it.
        $api  = $cfg.Server.TrimEnd('/')
        $site = if ($cfg.Baseline) { $cfg.Baseline -replace '/api/baseline\.json$', '' } else { $null }
        $ua   = "LevelPaceCompanion/$Version"

        $base = Invoke-RestMethod -Uri "$api/api/baseline" -UserAgent $ua -TimeoutSec 30
        Write-Baseline $addon $base $api

        $lb = Invoke-RestMethod -Uri "$api/api/leaderboard?board=levelling&limit=100" -UserAgent $ua -TimeoutSec 30
        $tw = $null; $ver = $null
        try { $tw = Invoke-RestMethod -Uri "$api/api/leaderboard?board=pvp&limit=100" -UserAgent $ua -TimeoutSec 20 } catch { }
        if ($site) {
            try { $ver = Invoke-RestMethod -Uri "$site/api/version.json" -UserAgent $ua -TimeoutSec 20 } catch { }
        }

        Write-Board $addon $lb.entries ($(if ($tw) { $tw.entries } else { @() })) $ver $api
